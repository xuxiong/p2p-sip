...
'''
A Sip caller application to initiate or receive VoIP calls from command line.
It using 39peer's SIP, SDP.
It calls external application to stream audio and video with RTP

@author: Aurelien BOUIN (from Kundan Singh)
@copyright: (c) 2014, Aurelien BOUIN. All Rights Reserved. See LICENSING for details.
...
if __name__ == '__main__': # parse command line options, and set the high level properties
    ...
    group1.add_option('',   '--port',    dest='port',    default=5060, type="int", help='listening port number for SIP UDP/TCP. TLS is one more than this. Default is 5060')
    ...
    group2.add_option('',   '--user-agent', dest='user_agent', default='sipstackcaller', help='set this as User-Agent header in outbound SIP request. Default is empty "" to not set')
    ...
    group2.add_option('',   '--pass',    dest='password',    default='')  
...
class Stacks(object):
    ...
    def authenticate(self, ua, header, stack):
      header.username = self.options.user + '@' + self.options.domain 
...
class Call(UA):
        ...
        if self.state == 'inviting':
            if response.is2xx:
                ...
                if response.body and response['Content-Type'] and response['Content-Type'].value.lower() == 'application/sdp':
                    ...
                    if self.media:
                        if not self.options.streams_loopback and self.options.audio:
                            self.startStreams()
    ...
    def receivedRequest(self, ua, request):
        if request.method == 'INVITE':
            if self.state == 'idle':
                ...
                self.media = voip.MediaSession(app=self, streams=self._audio_and_video_streams, request=req, listen_ip=self.options.int_ip, NetworkClass=rfc3550.gevent_Network) # create local media session
    ...
    def received(self, media, fmt, packet): # an RTP packet is received. Loop it back to caller.
        if self.options.streams_loopback:
